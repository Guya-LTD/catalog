...
from catalog.repository.product import Product
from catalog.repository.variant_type import VariantType
from catalog.repository.category import Category
from catalog.dto.product_dto import ProductDto
from catalog.blueprint.v1.product import namespace
...
from catalog.repository.embed import Names, Descriptions, Images, Assets, Pricing, Variant
...
class ProductList(Resource):
    ...
    def get(self):
        ...
        products = Product.objects(__raw__ = filters).order_by(order_by).paginate( page = page, per_page = limit).items
    ...
    @namespace.expect(ProductDto.request, validate = False)
    def post(self):
        """Save data/datas to database
        ...
        descriptions = Descriptions(
            ...
            am = namespace.payload.get('descriptions').get('am')
        )

        ## Variants
        variants = []
        for va in namespace.payload['variants']:
            #variant_type = VariantType.objects.get(id = va['variant_type_id'])
            variant_type = VariantType.objects.get(names__en = va['variant_name'])
            variant = Variant(
                variant_name = va['variant_name'],
                variant_type = variant_type,
                value = va['value'],
                additional_amount = va['additional_amount']
            )
            variants.append(variant)

        ## Category
        #category = Category.objects.get(id = namespace.payload['category_id'])
        category = Category.objects.get(names__en = namespace.payload['category_name'])
        category_name = namespace.payload['category_name']

        ## Assets
        images = Images(
            height = int(namespace.payload['images']['height']),
            width = int(namespace.payload['images']['width']),
            priority = int(namespace.payload['images']['priority']),
            src = namespace.payload['images']['src']
        )
        assets = Assets(
            images = [images]
        ) 
        ...
        ## Pricing
        pricing = Pricing(
            price = float(namespace.payload['pricing']['price'])
        )
        ...
        product = Product(
            names = names,
            descriptions = descriptions,
            sku = namespace.payload.get('sku'),
            variants = variants,
            #store_id = namespace.payload['store_id'],
            category = category,
        ...
        if isinstance(product.id, ObjectId):
            return make_response(jsonify({
                'status_code': 201,
                'status': 'Created'
            }), 201)
...
class ProductResource(Resource):
    """"Single Product Related Operation
    ...
    def put(self, id):
        ...
        # update sets
        product = Product.objects(id = id).update(
            names = names,
    ...
    def delete(self, id):
        ...
        products = Product.objects(id = id).delete()
